...
[Android View 的绘制（2D）](https://www.jianshu.com/p/21ac038a7deb)
...
[Paint 画笔路径效果](https://www.cnblogs.com/xqxacm/p/6669596.html)
...
调用canvas的`save`方法，之前绘制的内容都还在画布上，而调用`saveLayer`方法，之前绘制的内容就被保存在新的图层上，之后操作的图层就是没有内容的空图层；直到调用`restore`或者`restoreToCount`方法。